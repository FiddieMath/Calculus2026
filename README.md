...
给《微积分》课程用的 GitHub Pages 网页．主页保持简约：学生输入 **学号 + 查询码**（2026 秋查询码由助教发放，也可用学号邮箱自助查询）后，会**弹窗显示各次作业的完成情况**；主页下方是**作业安排表**（作业次数 / 截止时间 / 作业内容）与**附加资料表**．点击作业名称进入该次作业页面，**题目与参考答案都在里面更新**（公式由本地组件渲染，不依赖外网）．
...
4. 学生输入“学号 + 查询码”，弹窗中查看每次作业的完成情况、分数和批注（查询码由助教发放，隐私强度见第 6 节）．
...
└─ .github/workflows/
   ├─ pages.yml               # 网页自动部署
   └─ query-code-mailer.yml   # 长轮询邮件机器人（55 分钟/次，自触发接续，每小时兜底）
```
...
- **查询码规则**：2026 秋在册学生使用 6 位随机查询码（推荐做法，同学之间互不可见）；补选学生默认以学号作为查询码，若希望更私密，可用教师工具的“② 修改查询码”改成随机码后再发布．
...
原理：GitHub Actions 启动一次后运行约 55 分钟，期间每 60 秒检查一次查询邮箱的新邮件；本次运行结束会自动触发下一次，并有每小时一次的定时任务兜底（GitHub 的定时触发不保证准时，长轮询可以避免这个问题）．若发件地址严格匹配 `学号@smail.nju.edu.cn`，且该学号在本班查询码表中，就自动回复该学生自己的查询码．查询码表只保存在 GitHub Secret 中，**不进入公开仓库**．

邮件主题必须包含 **“查询码”** 三个字（例如主题写“查询码”即可），否则机器人会跳过．想修改或取消这一限制，可在仓库 Variables 中设置 `MAIL_SUBJECT_KEYWORD`：改成其它关键词即换成新关键词，设为 `-` 表示不限制主题．正文可以留空．
...
2. 用一份学校邮箱（或请学生协助）发一封测试邮件，主题写“查询码”．日志里应出现 `DRY-RUN 将回复：22*****53` 这类打码记录；日志不会显示学号全貌和查询码．
3. 确认无误后，再次运行工作流、不勾 dry_run．之后这次运行会持续约 55 分钟、每分钟检查一次，并在结束时自动触发下一次；学生发邮件通常一分钟左右就能收到查询码．

### 9.4 注意事项

- 机器人只下载邮件头、不读正文，也只把“真正成功回复过”的查询邮件和“同一学生本次重复来信”标记为已读；其它未读邮件（私人邮件、非本班学生邮件等）都不会被动到．
- 邮件机器人默认在结课日期 **2027-01-15** 后停止：当天仍正常回信，1 月 16 日起不再回信，并自动停用该工作流（若自动停用失败，请在 Actions 页面手动停用 `Query code mail bot`）．想改日期，在仓库 Variables 中添加 `MAIL_BOT_END_DATE`，格式如 `2027-01-15`．
...
- 机器人只处理“直接发给查询邮箱、发件人是本班学号邮箱、主题包含‘查询码’”的邮件，并跳过自动回复、退信、`Re:` 回复、主题不含关键词、学号不在名单等情况；日志会写明跳过原因（学号与邮箱地址打码）．
- 公开仓库的 Actions 日志是公开的，机器人只输出处理数量与打码学号，不输出查询码．
- 长轮询模式下，每次运行占用一个 Linux 运行器约 55 分钟（公开仓库的 Actions 时长免费）；如果因为仓库连续 60 天没有活动导致定时兜底被暂停，推送一次即可恢复．若你希望立即检查邮箱，也可以在 Actions 页面手动运行一次．
- 修改 `scripts/mail_query_bot.py` 或本工作流文件后，push 会自动启动一次长轮询，方便改动立即生效并处理积压邮件．
